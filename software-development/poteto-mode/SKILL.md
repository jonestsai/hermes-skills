...
- reflect-refs reviewer briefs (judgment-reviewer.md, tooling-reviewer.md, divergent-reviewer.md) are passed to children verbatim per reflect.md — children never see this table, and their "scan the transcript for skill reads against `.cursor/skills/`" checklist points at paths that don't exist here. When passing these briefs, the PARENT substitutes that checklist line: evidence of a skill used = `skill_view` calls, `read_file`/`search_files` hits under `~/.hermes/skills/` or `~/.hermes/profiles/<p>/skills/`, `delegate_task` context naming a skill, or tool calls matching a skill's documented commands. Everything else in the brief passes verbatim.
