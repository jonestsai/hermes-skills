...
- create-verification-skill output: LOCALIZED STRUCTURALLY — `references/workflow/create-verification-skill.md` and `references/workflow/maintain-verification-skill.md` now name the Hermes skills tree directly (`~/.hermes/skills/verification/verify-<app>/` by default, or `~/.hermes/profiles/<p>/skills/verification/verify-<app>/` when one profile owns the app), so no on-the-fly translation is needed. The generated frontmatter (name + description) registers it as a real Hermes skill, so agents can discover and trigger it; the feature map is plain files read via `read_file`. Generated verify skills are manual (never hub/tap-managed).
...
- reflect-refs reviewer briefs (judgment-reviewer.md, tooling-reviewer.md, divergent-reviewer.md): LOCALIZED — their skill-evidence checklist now names Hermes tools (`skill_view`/`read_file`, `delegate_task`, terminal/search_files) and `~/.hermes/skills/` paths directly, so they can be passed to children verbatim as reflect.md instructs. Verification-skill generator refs (`references/workflow/create-verification-skill.md`, `references/workflow/maintain-verification-skill.md`): LOCALIZED — they now name the Hermes skills tree directly, so generators write to the Hermes location without on-the-fly translation. These files are the port's only intentional upstream divergences among the reference files; on the next upstream sync, re-apply these localizations if upstream touches these paragraphs.
